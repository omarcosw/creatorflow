...
const ShotListManager: React.FC<ShotListManagerProps> = ({ shotLists, onSaveList, onDeleteList, onBack }) => {
  ...
  if (activeList) {
    return (
      <div className="flex flex-col h-full bg-zinc-950 animate-in fade-in duration-300 relative">
        <header className="px-6 py-4 border-b border-zinc-800 bg-zinc-900 sticky top-0 z-10">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setActiveListId(null)} className="p-2 -ml-2 hover:bg-zinc-800 rounded-full transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-white leading-tight">{activeList.title}</h1>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-4xl mx-auto space-y-3">
            {activeList.items.length === 0 ? (
              <div className="py-20 text-center text-zinc-400 border-2 border-dashed border-zinc-800 rounded-3xl">
              ...
              activeList.items.map((item, idx) => (
                <div key={item.id} className={`flex items-start gap-4 p-4 bg-zinc-900 border border-zinc-800 rounded-2xl transition-all shadow-sm ${item.isCompleted ? 'opacity-60 bg-zinc-950 border-emerald-500/30' : 'hover:border-emerald-500'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-black bg-zinc-800 px-2 py-0.5 rounded text-zinc-500 uppercase">Shot {idx + 1}</span>
                      <h3 className={`font-bold text-white truncate ${item.isCompleted ? 'line-through text-zinc-400' : ''}`}>{item.scene}</h3>
                    </div>
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 p-5 border-2 border-dashed border-zinc-800 rounded-2xl text-zinc-400 hover:border-emerald-500 hover:text-emerald-500 transition-all mt-6"
            >
        ...
        {isAddingShot && (
            <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-zinc-900 w-full max-w-md p-6 rounded-3xl shadow-2xl border border-zinc-800 animate-in slide-in-from-bottom-10 duration-300">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-white">Novo Take</h3>
                        <button onClick={() => setIsAddingShot(false)} className="p-1 hover:bg-zinc-800 rounded-full text-zinc-500"><X className="w-5 h-5" /></button>
                    </div>
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                              ...
                              className={`w-full bg-zinc-950 border rounded-xl p-4 text-white focus:outline-none focus:border-emerald-500 ${shotSceneError ? 'border-red-500' : 'border-zinc-800'}`}
                            />
                        ...
                        <div>
                            ...
                            <textarea 
                              ...
                              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-white focus:outline-none focus:border-emerald-500 h-24 resize-none"
                            />
                        </div>
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 animate-in fade-in duration-300">
      <header className="px-6 py-8 border-b border-zinc-800 bg-zinc-900 shadow-sm">
        <div className="max-w-5xl mx-auto w-full flex items-center justify-between mb-8">
            <button onClick={onBack} className="p-2 -ml-2 text-zinc-500 hover:hover:text-zinc-100 transition-colors">
                <ArrowLeft className="w-6 h-6" />
            </button>
            <div className="p-3 bg-emerald-900/30 text-emerald-600 rounded-2xl">
                <Video className="w-8 h-8" />
            </div>
        </div>
        <div className="max-w-5xl mx-auto w-full">
            <h1 className="text-4xl font-bold text-white mb-2">Lista de Gravação</h1>
            <p className="text-zinc-400 font-medium">Acompanhe seus takes no set. Tique o que já foi gravado.</p>
        </div>
      </header>
        ...
        <div className="max-w-5xl mx-auto w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <button 
                    ...
                    className="flex flex-col items-center justify-center p-8 bg-zinc-900 border-2 border-dashed border-zinc-800 rounded-3xl hover:border-emerald-500 group transition-all h-[200px]"
                >
                    <div className="p-4 rounded-xl bg-emerald-900/30 text-emerald-600 mb-4 group-hover:scale-110 transition-transform">
                ...
                {shotLists.map(list => {
                    ...
                    return (
                        <div key={list.id} className="group relative flex flex-col p-6 bg-zinc-900 border border-zinc-800 rounded-3xl shadow-sm hover:shadow-xl transition-all h-[200px]">
                            <button 
                                onClick={() => setActiveListId(list.id)}
                                className="flex-1 flex flex-col items-start w-full text-left"
                            >
                                <div className="p-2.5 bg-zinc-800 rounded-xl mb-4 group-hover:text-emerald-500 transition-colors">
                                    <Layers className="w-5 h-5" />
                                </div>
                                <h3 className="text-xl font-bold text-white mb-1 truncate w-full">{list.title}</h3>
      ...
      {isAddingList && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
              <div className="bg-zinc-900 w-full max-w-md p-6 rounded-3xl shadow-2xl border border-zinc-800">
                  <h3 className="text-xl font-bold text-white mb-6">Novo Vídeo para Gravar</h3>
                  <div className="space-y-4">
                      <div>
                          ...
                          <input
                            ...
                            className={`w-full bg-zinc-950 border rounded-xl p-4 text-white focus:outline-none focus:border-emerald-500 ${listNameError ? 'border-red-500' : 'border-zinc-800'}`}
                          />
