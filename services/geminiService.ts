...
export const transcribeAudio = async (audioBase64: string): Promise<string> => {
  try {
    ...
    // Initializing Gemini client properly
...
export const sendMessageToAgent = async (
  ...
  try {
    const apiKey = process.env.API_KEY || process.env.GEMINI_API_KEY;
    if (!apiKey) {
      return { text: "⚠️ API Key não encontrada. Adicione GEMINI_API_KEY no .env.local e reinicie o servidor." };
    }
    // Sempre criar uma nova instância para garantir a API Key mais recente (exigência Veo/Pro)
    const ai = new GoogleGenAI({ apiKey });
    const { temperature, useThinking, useSearch } = getAgentConfig(agentId);
    
    // Storyboard usa o modelo Pro (Nano Banana Pro) para ilustração automática
    // IMAGE_GENERATOR removido do isProVisual para focar apenas em prompts de texto
    ...
    // Para modelos de imagem, o sistema de instrução deve vir no prompt ou histórico
    history.forEach(msg => {
    ...
    // Configuração específica para geração de imagem com Nano Banana Pro
    if (isProVisual) {
        config.imageConfig = {
            aspectRatio: "9:16", // Foco em Storyboard Vertical
            imageSize: imageSize
        };
    } else {
    ...
    // Pro models also support search grounding as per guidelines
    if (useSearch) {
    ...
    const hint = rawMessage.includes("API key") || rawMessage.includes("API_KEY") || rawMessage.includes("invalid")
      ? "Verifique se a chave está correta, com o serviço Gemini API habilitado e sem restrições de HTTP Referrer (adicione http://localhost:3001/*)."
